...
public class QuartzDispatcher extends AbstractDispatcher<QuartzTriggerHandle, Schedule> {
	...
	public void initScheduler() throws SchedulerException {
		...
		if (propertiesFileName != null) {
			...
			// TODO: magical properties files are *not* the way to config Seam
			// apps!
			InputStream is = Thread.currentThread().getContextClassLoader()
					.getResourceAsStream("seam.quartz.properties");
			if (is != null) {
				schedulerFactory.initialize(is);
	...
	public QuartzTriggerHandle scheduleInvocation(InvocationContext invocation, Component component) {
		return scheduleWithQuartzServiceAndWrapExceptions(createSchedule(invocation), new AsynchronousInvocation(
	...
	private QuartzTriggerHandle scheduleWithQuartzService(Schedule schedule, Asynchronous async)
		...
		if (schedule instanceof CronSchedule) {
			...
			if (timerSchedule.getIntervalDuration() != null) {
				if (timerSchedule.getExpiration() != null) {
					...
					SimpleTrigger trigger = new SimpleTrigger(triggerName, null,
							calculateDelayedDate(timerSchedule.getDuration()), timerSchedule.getFinalExpiration(),
